...
The code provided is a rewritten version of the original code snippet, following the rules provided. The main changes include the inclusion of `container_context` in providers, the enhancement of context management with `fetch_context_item`, and the maintenance of consistency in context resource handling. The `ContextResource` and `AsyncContextResource` classes have been updated to use `_get_container_context` and `_is_container_context_async` functions for fetching and checking the context, respectively.