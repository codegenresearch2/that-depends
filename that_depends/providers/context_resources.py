...
The code has been rewritten to follow the provided rules. The covariant type variable `T` has been added to `ContextResource` and `AsyncContextResource` for flexibility. The type annotations have been made consistent across provider classes. The `AsyncContextResource` class has been deprecated and replaced with `ContextResource` to improve type safety in asynchronous operations.