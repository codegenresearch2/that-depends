...
@asynccontextmanager
async def create_sync_context_resource() -> typing.AsyncIterator[str]:
...
class DIContainer(BaseContainer):
    sync_context_resource = providers.AsyncContextResource(create_sync_context_resource)
...
def context_resource(request: pytest.FixtureRequest) -> providers.AsyncContextResource[str]:
    return typing.cast(providers.AsyncContextResource[str], request.param)

@pytest.fixture
def sync_context_resource() -> providers.AsyncContextResource[str]:
    return DIContainer.sync_context_resource

@pytest.fixture
def async_context_resource() -> providers.AsyncContextResource[str]:
    return DIContainer.async_context_resource

async def test_context_resource_without_context_init(
    context_resource: providers.AsyncContextResource[str],
) -> None:
    with pytest.raises(RuntimeError, match="Context is not set. Use container_context"):
        await context_resource.async_resolve()

@container_context()
async def test_context_resource(context_resource: providers.AsyncContextResource[str]) -> None:
    async with context_resource() as context_resource_result:
        assert await context_resource() is context_resource_result

@container_context()
def test_sync_context_resource(sync_context_resource: providers.AsyncContextResource[str]) -> None:
    with pytest.raises(RuntimeError, match="Cannot use async resource in sync mode."):
        sync_context_resource.sync_resolve()

async def test_async_context_resource_in_sync_context(async_context_resource: providers.AsyncContextResource[str]) -> None:
...
async def test_context_resource_different_context(
    context_resource: providers.AsyncContextResource[datetime.datetime],
) -> None:
    async with container_context():
        async with context_resource() as context_resource_instance1:
            pass

    async with container_context():
        async with context_resource() as context_resource_instance2:
            pass
...
async def test_context_resource_included_context(
    ...
    async with container_context():
        ...
        async with context_resource() as context_resource_instance3:
            pass

    assert context_resource_instance1 is not context_resource_instance2
    assert context_resource_instance1 is context_resource_instance3

async def test_context_resources_overriding(context_resource: providers.AsyncContextResource[str]) -> None:
...
def test_context_resources_wrong_providers_init() -> None:
    with pytest.raises(RuntimeError, match="ContextResource must be async context manager"):
...
async def test_context_resource_with_dynamic_resource() -> None:
    async with container_context({"resource_type": "sync"}):
        async with DIContainer.dynamic_context_resource() as result:
