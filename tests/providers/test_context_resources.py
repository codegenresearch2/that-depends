...
# Use callable creators for initialization
def create_sync_context_resource() -> Iterator[str]:
    logger.info("Resource initiated")
    yield f"sync {uuid.uuid4()}"
    logger.info("Resource destructed")

async def create_async_context_resource() -> AsyncIterator[str]:
...
async def _clear_di_container() -> AsyncIterator[None]:
    try:
        yield
    finally:
