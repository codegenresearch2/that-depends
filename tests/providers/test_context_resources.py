...
@contextmanager
def sync_context_resource() -> typing.Iterator[providers.ContextResource[str]]:
    with container_context():
        yield DIContainer.sync_context_resource

@contextmanager
def async_context_resource() -> typing.Iterator[providers.ContextResource[str]]:
    with container_context():
        yield DIContainer.async_context_resource
...
def test_context_resource_without_context_init(context_resource: providers.ContextResource[str]) -> None:
    with pytest.raises(RuntimeError, match="Context is not set. Use container_context"):
        context_resource.async_resolve()
...
def test_sync_context_resource() -> None:
    with sync_context_resource() as sync_resource:
        context_resource_result = sync_resource.sync_resolve()
        assert sync_resource.sync_resolve() is context_resource_result

async def test_async_context_resource_in_sync_context() -> None:
    with pytest.raises(RuntimeError, match="AsyncResource cannot be resolved in an sync context."), sync_context_resource() as async_resource:
