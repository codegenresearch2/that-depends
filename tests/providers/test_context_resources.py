...
class DIContainer(BaseContainer):
    ...
    async def tear_down(self):
        if self.is_async_func(self.sync_context_resource.provider):
            await self.sync_context_resource.async_tear_down()
        else:
            self.sync_context_resource.sync_tear_down()

        if self.is_async_func(self.async_context_resource.provider):
            await self.async_context_resource.async_tear_down()
        else:
            self.async_context_resource.sync_tear_down()

@pytest.fixture(autouse=True)
async def clear_di_container() -> typing.AsyncIterator[None]:
    try:
        yield
    finally:
