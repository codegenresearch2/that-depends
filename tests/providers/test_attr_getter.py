...
@pytest.mark.asyncio
async def test_nesting_levels_sync(field_count: int, test_field_name: str, test_value: str | int) -> None:
    obj = NestingTestDTO()
...
@pytest.mark.asyncio
async def test_attr_getter_with_invalid_attribute_sync(some_settings_provider_sync: providers.Singleton[Settings]) -> None:
    with pytest.raises(AttributeError):
        some_settings_provider_sync().nested1_attr.nested2_attr.__some_private__  # noqa: B018
    with pytest.raises(AttributeError):
        some_settings_provider_sync().nested1_attr.__another_private__  # noqa: B018
    with pytest.raises(AttributeError):
        some_settings_provider_sync().nested1_attr._final_private_  # noqa: B018


@pytest.mark.asyncio
async def test_attr_getter_with_invalid_attribute_async(some_settings_provider_async: providers.Singleton[Settings]) -> None:
    with pytest.raises(AttributeError):
        some_settings_provider_async().nested1_attr.nested2_attr.__some_private__  # noqa: B018
    with pytest.raises(AttributeError):
        some_settings_provider_async().nested1_attr.__another_private__  # noqa: B018
    with pytest.raises(AttributeError):
        some_settings_provider_async().nested1_attr._final_private_  # noqa: B018