...
from that_depends.providers.base import Singleton, AbstractProvider, AttrGetter
...
@pytest.fixture
async def some_settings_provider() -> Singleton[Settings]:
    return Singleton(Settings)

async def test_attr_getter_with_zero_attribute_depth(some_settings_provider: Singleton[Settings]) -> None:
    attr_getter = some_settings_provider.some_str_value
    assert await attr_getter.async_resolve() == Settings().some_str_value

async def test_attr_getter_with_more_than_zero_attribute_depth(some_settings_provider: Singleton[Settings]) -> None:
...
def test_nesting_levels(field_count: int, test_field_name: str, test_value: str | int) -> None:
    ...
    attr_value = AttrGetter._get_value_from_object_by_dotted_path(obj, attr_path)
    assert attr_value == test_value

async def test_attr_getter_with_invalid_attribute(some_settings_provider: Singleton[Settings]) -> None:
