...
from that_depends.providers.base import AttrGetter
...
@pytest.fixture
def settings_provider() -> providers.Singleton[Settings]:
    return providers.Singleton(Settings)

def test_attr_getter_with_zero_attribute_depth(settings_provider: providers.Singleton[Settings]) -> None:
    attr_getter = AttrGetter(settings_provider, "some_str_value")
    assert attr_getter.sync_resolve() == Settings().some_str_value

def test_attr_getter_with_more_than_zero_attribute_depth(settings_provider: providers.Singleton[Settings]) -> None:
    attr_getter = AttrGetter(settings_provider, "nested1_attr.nested2_attr.some_const")
...
def test_nesting_levels(field_count: int, test_field_name: str, test_value: str | int) -> None:
    ...
    attr_value = AttrGetter(providers.Singleton(obj), attr_path).sync_resolve()
    assert attr_value == test_value

def test_attr_getter_with_invalid_attribute(settings_provider: providers.Singleton[Settings]) -> None:
    with pytest.raises(AttributeError):
        AttrGetter(settings_provider, "nested1_attr.nested2_attr.__some_private__")
    with pytest.raises(AttributeError):
        AttrGetter(settings_provider, "nested1_attr.__another_private__")
    with pytest.raises(AttributeError):
        AttrGetter(settings_provider, "nested1_attr._final_private_")